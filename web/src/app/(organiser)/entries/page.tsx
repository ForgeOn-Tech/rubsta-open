...
export default async function EntriesPage({
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <div className="eyebrow">{tournament.name} · Organiser</div>
        <h1 className="mt-2 text-[24px] font-semibold tracking-[-0.02em]">
          Entries
        </h1>
        <div className="mono mt-3 flex flex-wrap gap-x-5 gap-y-1 text-[11px] uppercase text-dim">
          <span>{rows.length} entries</span>
          <span>Fee {formatFee(tournament.feeCents, tournament.currency)}</span>
          <span>Closes {formatEntryCloses(tournament.entryClosesAt)} IST</span>
        </div>
      </div>
